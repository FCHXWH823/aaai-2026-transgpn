...
$\mathcal{G}_t = G_t \cup C$, where $G_t = (V_t, E_t)$ is the partial
synthesized network and $C = G_{\mathrm{SOP}}(g)$ is the fixed compensate
...
degree in $G_t$ and in $C$ separately, and a coverage indicator; edges carry
...
$\mathcal{G}_t = G_t \cup C_{\mathrm{SOP}}$; four factored heads with per-head
safety masks decode the transistor placement $(u^*, v^*, k^*, s^*)$.}
\label{fig:sgpn}
\end{figure}

The \SGPN encodes $\mathcal{G}_t$ with an MPNN backbone~\cite{gilmer2017mpnn}
...
\begin{equation}
\begin{aligned}
  &\log \pi(a \mid \mathcal{G}_t) = \log p_1(u) + \log p_2(v \mid u) \\
...
\]
where $\mathcal{G}_{t-1}$ is built from the prefix $G_{t-1}^*$ and the fixed
