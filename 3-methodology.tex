...
\centering
\input{fig/sgpn_arch}
\caption{\textbf{(a)} A transistor network is a switching graph: an input
pattern yields a conducting source-to-sink ($S{\to}T$) path exactly on the
on-patterns of the target (coverage), and no path on the off-patterns
(safety). \textbf{(b)} The \sgpn (\SGPN) synthesizes such a network one
transistor at a time. A shared MPNN backbone encodes the unified graph
$H_t = G_t \cup C_g$ (the partial network fused with the fixed SOP target);
the four heads then fire \emph{sequentially}---source $u$, drain $v$, variable
$k$, sign $s$---each conditioned on the earlier selections and its own safety
mask. The placed transistor $(u^*,v^*,k^*,s^*)$ is appended to $G_t$ and the
step repeats until the network covers the target.}
\label{fig:sgpn}
\end{figure}
...
next $u$, so Head~1 becomes structurally determined rather than convention-dependent.
...
\label{eq:phi}
\end{equation}
whose weights $(w_c, w_m, w_w, w_d, w_g) = (3, 4, 1, 4, 2)$ match ASTRAN's
simulated-annealing cost exactly.  Wire length $W_{\mathrm{wl}}$ and routing
density $D_{\mathrm{rt}}$ require a routed cell, so our physical experiments
optimize and report only the three \emph{exact} placement metrics,
$\Phi_{\mathrm{plc}} = W_{\mathrm{cpp}} + N_{\mathrm{gm}} + N_{\mathrm{gap}}$
(unit weights, $w_w{=}w_d{=}0$); the full five-metric $\Phi$ under real ASTRAN
...
difference from the Phase~2 loop; App.~D gives the full training pseudocode for
all three phases.
