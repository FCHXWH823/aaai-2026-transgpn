...
\caption{Optimality rate (fraction of targets reaching the exact SAT optimum).
         Rates marked $\dagger$ ($N{=}4$, $N{=}6$) are over the evaluated
         subset of each sweep.}
\label{tab:main}
...
\noindent
MiniTNtk is an exact SAT-based solver that supplies the reference
...
($\approx 1.6$\,s), since rollout generation dominates while the baseline and
exploration terms are nearly free.  Throughput is instead governed by device
sharing---per-update time grows from $1.9$\,s at one process per GPU to $49$\,s
at sixteen, as each step issues many small kernels that serialize---so variant
comparisons are meaningful only at matched concurrency.
...
instead selects, among the many count-optimal realizations of a function,
those that also place compactly, lowering mean placed-cell cost by $18\%$
