...
placement cost of count-optimal networks (the Phase-2 objective) against the
Phase-3 physically-optimized networks, scored by ASTRAN's exact placement
metrics: cell width (CPP), gate mismatches (GM), and diffusion gaps, summed as
$\Phi_{\mathrm{plc}}$.  Phase~3 lowers the mean cell cost by $18\%$
($12.74\!\to\!10.44$): among the many count-optimal realizations of a
function, it selects those that also place compactly, confirming that
transistor count and layout quality are distinct objectives.

\begin{table}[t]
\centering
\caption{Placement cost over the $254$ three-input cells (mean ASTRAN
...
\midrule
Count-optimal networks (Phase~2) & $12.74$ \\
