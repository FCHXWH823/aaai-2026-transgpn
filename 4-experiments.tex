...
\begin{itemize}[leftmargin=*, topsep=2pt, itemsep=0pt]
  \item \textbf{MiniTNtk}~\cite{miniTNtk}: SAT-based exact
        transistor-count minimization (the source of our reference optima).
  \item \textbf{MuSTNet}~\cite{muSTNet}: SAT-based exact synthesis with
        placement awareness.
...
\begin{table}[t]
\centering
\caption{Physical placement over the $254$ three-input cells: mean ASTRAN cost
         $\Phi_{\mathrm{plc}} = \mathrm{CPP}+\mathrm{GM}+\mathrm{Gaps}$ of
         MiniTNtk's count-optimal cells vs.\ our physically-optimized cells
         (lower is better).}
\label{tab:physical}
\begin{tabular}{lc}
\toprule
\textbf{Method} & \textbf{Mean $\Phi_{\mathrm{plc}}$} \\
\midrule
MiniTNtk~\cite{miniTNtk} (count-optimal) & $12.74$ \\
\method{} Phase~3 (ours)                 & $\mathbf{10.44}$ \\
\bottomrule
\end{tabular}
\end{table}

The $4$-input evaluation over the frozen $256$-target set, and the
routing-aware objective (adding wire length and routing density under real
ASTRAN routing), are ongoing.
