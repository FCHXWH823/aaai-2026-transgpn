...
(a) The XOR SOP network of Example~\ref{ex:sop}: two shared-rail chains,
correct and coverage-complete, and reachable in any insertion order
...
(c) The AND2 cell decomposes into a parallel NMOS pull-down and a series PMOS
pull-up --- duals synthesized by one policy (Example~\ref{ex:dual}).
(d) A structurally dead transistor (grey) that is free to delete with no
off-pattern recheck (Example~\ref{ex:reduction}).
Circles marked S/T are the source/sink rails; small green nodes are internal.}
\label{fig:examples}
\end{figure}
...
By Proposition~\ref{prop:dual} the CMOS cell splits into two independent
switching problems.
The \emph{PDN} realizes $\lnot f$ with on-patterns $\Poff$: it must conduct
whenever $x_1{=}0$ or $x_2{=}0$, which is the \emph{parallel} pair of NMOS
switches $x_1 \,\|\, x_2$ between $\mathrm{GND}$ and $\mathrm{ZN}$ ---
i.e.\ a NAND-style pull-down.
The \emph{PUN} realizes $f(\lnot\mathbf{x})$ with on-patterns
$\{\lnot 11\}=\{00\}$: it must conduct only when $x_1{=}0$ and $x_2{=}0$,
which is the \emph{series} pair of PMOS switches $x_1$ then $x_2$ between
$\mathrm{VDD}$ and $\mathrm{ZN}$.
The two networks are duals (series $\leftrightarrow$ parallel) and are
synthesized by the \emph{same} policy conditioned on their respective pattern
sets, exactly as our per-target formulation exploits.
\end{example}
